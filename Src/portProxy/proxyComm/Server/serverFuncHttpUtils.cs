...
namespace Proxy.Comm
{
    ...
    /// {0}同一是服务器的url地址
    /// </summary>
    public class serverFuncHttpUtils
    {
        ...
        /// </summary>
        [Obsolete]
        private string regionMasterNoticeUrl = "/sysconf/gserver/NoticeServer";
        ...
        /// </summary>
        private string registerForZoneUrl = "{0}/sysconf/gserver/registerToZoneMaster";
        ...
        /// <summary>
        /// 向集群主服务器报告自己有变化，post请求，主控Id，和proxynettyServer
        /// </summary>
           private string reportServerChangedUrl = "{0}/sysconf/gserver/reportServerChanged/{1}";
        #endregion
        private  bool checkOk(JObject jobj)
        {
            return FrmLib.Extend.tools_static.jobjectHaveKey(jobj, "Status") && jobj["Status"].ToObject<int>() == 100;
        }
        ...
        public  string getEchoServerId(string urls)
        {
            ...
            {
                Console.WriteLine("getEchoServerId ex:" + FrmLib.Extend.tools_static.getExceptionMessage(ex));
            }
            return null;
        }
        ...
        public void reportMyChangeToMaster( )
        {


            var jobj = JObject.FromObject(ownServer);
             
              
                var respone = hclient.doAsycHttpRequest(string.Format(reportServerChangedUrl, ownCluser.master.serviceUrl, ownCluser.master.id),jobj, false,ownCluser.master.serviceUrl.ToLower().StartsWith("https"));
                ...
                {
                    FrmLib.Log.commLoger.runLoger.Error(string.Format("server {0} reportMyChangeToMaster {1} error ,resopne:{2}", ownServer.id, ownCluser.master.serviceUrl, respone.ToString()));
                }
