...
namespace DotNetty.Codecs.Http
{
    ...
    public  class HttpProxyObjectDecoder : ByteToMessageDecoder
    {
        ...
        protected  IHttpMessage CreateMessage(AsciiString[] initialLine) {
         return   new DefaultHttpRequest(
              new HttpVersion(initialLine[2].ToString(),false),
              HttpMethod.ValueOf(initialLine[0]), initialLine[1].ToString(), this.ValidateHeaders);
        }

        protected  IHttpMessage CreateInvalidMessage() => new DefaultFullHttpRequest(HttpVersion.Http10, HttpMethod.Get, "/bad-request", this.ValidateHeaders);

        protected  bool IsDecodingRequest() => true;

        protected HttpProxyObjectDecoder() : this(4096, 8192, 8192, true)
        {
        }

        protected HttpProxyObjectDecoder(
        ...
        private string getAppKeyFromUri(string uri)
        {
            ...
            if (pos2 < 0 || pos2<pos1+1)
                return null;
            return tmp.Substring(pos1 + 1, pos1 - pos2);
        }
        private void copyByteBufferToPho(IByteBuffer buffer)
        {
        ...
        protected bool IsSwitchingToNonHttp1Protocol(IHttpResponse msg)
        {
            ...
            return !msg.Headers.TryGet(HttpHeaderNames.Upgrade, out ICharSequence newProtocol)
                || !AsciiString.Contains(newProtocol, Http10String) 
                   && !AsciiString.Contains(newProtocol, Http11String);
        }
